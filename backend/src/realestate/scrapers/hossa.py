...
def _street_from_address_node(address_el) -> str | None:
    if address_el is None:
        return None
    parts = [span.text(strip=True).strip(" ,/") for span in address_el.css("span")]
    parts = [part for part in parts if part and part.lower() != "ul."]
    if parts:
        if len(parts) == 2 and re.fullmatch(r"\d+[a-zA-Z]?", parts[1]):
            return " ".join(parts)
        return ", ".join(parts)
    text = address_el.text(separator=" ", strip=True)
    text = re.sub(r"\bul\.\s*", "", text, flags=re.IGNORECASE)
    text = re.sub(r"\s+", " ", text).strip(" ,/")
    return text or None


def _full_address(title: str, city: str | None, street: str | None) -> str | None:
    parts = [part for part in (title, city, street) if part]
...
class HossaScraper:
    ...
    def parse_search(self, html: str) -> list[RawListing]:
        ...
        for card in tree.css(".o-card__inner"):
            ...
            street = _street_from_address_node(address_el)
            ...
            listings.append(
                RawListing(
                    ...
                    attributes={
                        ...
                        "investment_url": url,
                        "address": _full_address(text, city, street),
                    },
                )
            )
    ...
    def _parse_apartments_api(self, body: str, url: str) -> list[RawListing]:
        ...
        for item in rows:
            ...
            city = item.get("city") or item.get("investment_city")
            district = item.get("district") or item.get("investment_district")
            street = item.get("street") or item.get("building") or item.get("investment_street")
            listings.append(
                RawListing(
                    source_id=self.source_id,
                    external_id=f"apartment-{item_id}",
                    url=(
                        f"{_BASE_URL}/{investment}/wyniki-wyszukiwania/"
                        f"#id={item_id}&inv={investment}"
                    ),
