...
class _FailingEmbedClient:
    async def embed(self, texts):
        raise RuntimeError("provider rejected embeddings request")
...
async def test_hybrid_degrades_to_rule_based_when_embedding_fails(engine):
    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)
    async with AsyncSession(engine, expire_on_commit=False) as s:
        dim = get_embedding_dim()
        await _listing(s, ext="a", vec=[0.0] * dim)
        svc = SearchService(s, client=_FailingEmbedClient())
        items, total = await svc.search_hybrid(ListingFilters(nl_query="cokolwiek"))
        assert total == 1
