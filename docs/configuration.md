...
Konfiguracja aplikacji odbywa się przez zmienne środowiskowe lub plik `.env` w katalogu głównym projektu (wzór: `.env.example`). Klasa `Settings` w `src/realestate/config.py` używa `pydantic-settings` do walidacji i ładowania ustawień.
...
| `EMBEDDING_DIM` | nie | `1536` | Wymiar wektora pgvector. **Musi być identyczny przy migracji i przy uruchomieniu aplikacji.** Jedyne źródło prawdy: `get_embedding_dim()` w `config.py`. |
